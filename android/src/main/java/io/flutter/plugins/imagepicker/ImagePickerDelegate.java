...
public class ImagePickerDelegate
  ...
  private void handleChooseImageResult(int resultCode, Intent data) {
    if (resultCode == Activity.RESULT_OK && data != null) {
      ...
      finishWithSuccessForMultiple(finalImagePath);
      return;
    }
  ...
  private void finishWithSuccessForMultiple(ArrayList<String> imagePaths) {
    for (int i = 0; i < imagePaths.getItemCount(); i++) {
      String imagePath = imagePaths.getItemAt(i);
      cache.saveResult(imagePath, null, null);
    }
    clearMethodCallAndResult();
    return;
